...
def uNFWtr_fct(rbin,cvir,t,Mvir,param):
    ...
    rvir = (3.0*Mvir/(4.0*np.pi*DELTAVIR*cosmo.rhoc_of_z()))**(1.0/3.0)
    x = cvir*rbin/rvir
    return 1.0/(x * (1.0+x)**2.0 * (1.0+x**2.0/t**2.0)**2.0)
...
def rhoNFW_fct(rbin,cvir,Mvir,param):
    ...
    cosmo = CosmoCalculator(param)
    rvir = (3.0*Mvir/(4.0*np.pi*DELTAVIR*cosmo.rhoc_of_z()))**(1.0/3.0)
    rho0 = DELTAVIR*rhoc_of_z()*cvir**3.0/(3.0*np.log(1.0+cvir)-3.0*cvir/(1.0+cvir))
    x = cvir*rbin/rvir
    return rho0/(x * (1.0+x)**2.0)
...
def MNFWtr_fct(rr,cvir,t,Mvir,param):
    """
    Truncateed NFW mass profile.
    """
    cosmo = CosmoCalculator(param)
    rvir = (3.0*Mvir/(4.0*np.pi*DELTAVIR*cosmo.rhoc_of_z()))**(1.0/3.0)
    return Mvir*mNFWtr_fct(cvir*rr/rvir,t)/mNFWtr_fct(cvir,t)
...
def profiles(rbin,Mvir,cvir,cosmo_corr,cosmo_bias,cosmo_var,param):
    ...
    aa = 1/(1+zz)
    cosmo = CosmoCalculator(param)
    Da = cosmo.growth_factor(aa)
    D0 = cosmo.growth_factor(1.0)
    Da = Da/D0
    peak_height = 1.686/cosmo_var**0.5
    eps  = (eps0 - eps1*peak_height)
    ...
    #Initial density and mass profiles
    rho0NFWtr = DELTAVIR*cosmo.rhoc_of_z()*cvir**3.0/(3.0*mNFWtr_fct(cvir,tau))
